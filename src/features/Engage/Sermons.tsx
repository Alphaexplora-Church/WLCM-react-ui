...
export default function Sermons() {
  ...
  const filteredSermons = useMemo(() => {
    if (!searchQuery.trim()) return ALL_MORE_SERMONS;
    const lower = searchQuery.toLowerCase();
    return ALL_MORE_SERMONS.filter(
      (s) => s.title.toLowerCase().includes(lower) || s.series.toLowerCase().includes(lower)
    );
  }, [searchQuery]);
  ...
  const moreSermons = filteredSermons.slice(pageStart, pageStart + SERMONS_PER_PAGE);
  ...
  return (
    <>
      <Navigation />
          ...
          <section className="relative flex flex-col overflow-hidden bg-midnight-teal pt-28 pb-10 md:pt-36 md:pb-14">        <span
      ...
      <div className="relative z-10 mx-auto mb-8 max-w-7xl px-6 md:mt-20 md:px-12 w-full">          <motion.div
        ...
        <div className="mx-auto max-w-7xl px-6 md:px-12">
          <div className="mb-12 flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <div>
              ...
              <p className="max-w-xl font-sans text-sm text-midnight-teal/50">
            ...
            <div className="relative w-full md:w-72">
              <input
                ...
                className="w-full rounded-full border border-midnight-teal/15 bg-white px-5 py-3 pr-11 font-sans text-sm text-midnight-teal outline-none transition-colors focus:border-harvest-orange"
              />
              <svg
                ...
                strokeWidth={2}
                stroke="currentColor"
                className="absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-midnight-teal/40"
              >
